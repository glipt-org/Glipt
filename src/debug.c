...
int disassembleInstruction(Chunk* chunk, int offset) {
    ...
    switch (instruction) {
        ...
        case OP_ALLOW: {
            ...
            const char* names[] = {"exec", "net", "read", "write", "env"};
            printf("%-16s %s ", "OP_ALLOW", permType < 5 ? names[permType] : "?");
            printValue(chunk->constants.values[constant]);
            printf("\n");
            return offset + 3;
        }
        ...
        case OP_IMPORT: {
            uint8_t pathConst = chunk->code[offset + 1];
            uint8_t nameConst = chunk->code[offset + 2];
            printf("%-16s %4d '", "OP_IMPORT", pathConst);
            printValue(chunk->constants.values[pathConst]);
            printf("' as '");
